class_name CameraEffectsNetfox
extends Node3D
...
# 	_add_recoil(-recoil.y, -recoil.x)
# 	_add_screen_recoil(-0.2)

## Called via the weapon "fired" signal
func on_weapon_fired():
	_add_screen_recoil(-0.2)

func _add_screen_recoil(amount: float):
	target_position.z += amount

func _add_recoil(pitch: float, yaw: float) -> void:
