...
@export var step_speed_threshold := 4.0
...
func _physics_process(delta: float) -> void:
	...
	if (player_horizontal_speed > step_speed_threshold && player.state_controller.current_state == StateController.PlayerState.GROUNDED):
		# player is moving here
		step_timer += delta
		var step_frequency = step_rate / player_horizontal_speed
